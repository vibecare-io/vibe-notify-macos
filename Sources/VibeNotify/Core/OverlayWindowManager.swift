...
public class OverlayWindowManager {
  ...
  private func createBlurWindow(configuration: Configuration, notificationId: UUID) -> NSWindow {
    ...
    window.level =
      ...
      ? NSWindow.Level(rawValue: Int(NSWindow.Level.floating.rawValue) - 1)
    ...
    if let intensity = configuration.screenBlurIntensity {
      ...
      let tapView = NSHostingView(
        rootView: FullScreenBlurView(
          intensity: intensity,
      ...
      window.contentView = tapView
    } else {
      // Legacy NSVisualEffectView-based blur
      window.backgroundColor = .clear
      let blurView = NSHostingView(
        rootView: FullScreenBlurView(
  ...
  private func calculateFrameForPosition(
    _ position: WindowPosition, size: CGSize, on screen: NSScreen
  ) -> CGRect {
    let screenFrame = screen.frame
    let padding: CGFloat = 20
    let x: CGFloat
